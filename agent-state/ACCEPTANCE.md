...
| G1-SEGELF | MC, ELF writer/parser, and LLD tests prove `R_386_SEG16`, `R_386_SUB16`, `R_386_SUB32`, `R_386_HUGE8`, malformed cases, overflow, retention, and semantics. | partial at `0af18ce9830252ddf97f4a68c0d2947b1af4cd02`: bounded `R_386_SEG16` overflow repair passed independent review; selected tests pass 2/2, the object scan shows one `R_386_SEG16`, `S=0xfffff` writes `0xffff`, and `S=0x100000` is diagnosed; malformed, retention, protected-mode rejection, and remaining relocation matrix pending |
...
Run `IA-16 — Resume bounded Gate 1 SEGELF matrix` as one bounded acceptance
task. Continue with the remaining malformed-input, relocation-retention,
protected-mode rejection, and overflow/semantics cases for the frozen SEGELF
family. Preserve the independently accepted `R_386_SEG16` overflow repair and
stop on any new implementation defect rather than repairing it in the matrix
task.
